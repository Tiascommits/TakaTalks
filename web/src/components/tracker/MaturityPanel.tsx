...
import { INSTRUMENT_LABELS, type InvestmentEntryDTO } from "./types";
    ...
    <section className="bg-card border border-line p-5">
      ...
      {reinvestableCash > 0 && (
        <div className="bg-[#EFF6F1] border border-green px-3.5 py-3 mb-3.5 text-sm">
          ...
          পুনঃবিনিয়োগযোগ্য নগদ (নিশ্চিত হওয়া পেমেন্ট থেকে)
        </div>
      )}

      {matured.length > 0 && (
        <div className="mb-4">
          <p className="text-xs font-semibold text-red mb-2">মেয়াদ শেষ, পেমেন্ট নিশ্চিত করো:</p>
      ...
      {upcoming.length > 0 ? (
            ...
            return (
              <li key={e.id} className="flex justify-between items-center py-2 text-sm">
                <div>
                  ...
                  <span className="text-muted text-xs">({INSTRUMENT_LABELS[e.instrumentType]})</span>
                </div>
                <div className="text-right">
                  <div className="font-mono">{fmtTaka(e.principalAmount)}</div>
                  <div className="text-[11px] text-muted">{days} দিন বাকি</div>
                </div>
              </li>
            );
          })}
        ...
        matured.length === 0 && (
          <p className="text-sm text-muted">সামনের ৯০ দিনে কোনো বিনিয়োগের মেয়াদ শেষ হচ্ছে না।</p>
        )
      )}
    </section>
  );
...
function MaturedRow({
  ...
  return (
    <li className="flex flex-wrap items-center gap-2 bg-[#FBEFEF] border border-red px-3 py-2 text-sm">
      ...
      <span className="text-xs text-muted">({INSTRUMENT_LABELS[entry.instrumentType]})</span>
